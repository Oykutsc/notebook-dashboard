...
an_files = [f for f in listdir("project1/data") if isfile(join("project1/data", f))]
route = ["project1/data/" + sub for sub in an_files]
...
pklf = open("project1/package_list.txt", "r")
package_list=[]
for line in pklf:
...
def func(flag_language):
    ...
    if flag_language == "r":
        ...
        fig.update_layout(barmode='stack', width=1450)
...
def cell_count_plot(flag_language):
    ...
    cell_count_fig.update_layout(barmode='stack', height=width, margin={
        "l": 60,
        "r": 10,
        "b": 0,
        "t": 20,
...
def index():
    feature = "both"
    plot1 = language_plot()
    plot2 = cell_count_plot(feature)
    plot3 = func(feature)
    df = full_set.to_html(table_id="example")
    return render_template('index.html', cell_count_graph=plot2, language_graph=plot1, libpy_plot=libpy_plot(), 
    libr_plot=libr_plot(), func_plot=plot3, df=df, dataset_plot=dataset_plot())


@app.route('/language_cell_count', methods=['GET', 'POST'])
def change_features():
...
@app.route('/language_func', methods=['GET', 'POST'])
def change_language():
